...
train_set = [
    ...
    if x.startswith("day") and x.split(".")[0] in train_days
]
valid_set = [
    data_path + df_valid + x
    for x in os.listdir(data_path + df_valid)
    if x.startswith("day") and x.split(".")[0] in valid_days
]
# else:
...
proc = Workflow(
    cat_names=cat_names, cont_names=cont_names, label_name=["label"], to_cpu=to_cpu
)
...
if int(args.freq_thresh) == 0:
    ...
    proc.add_preprocess(
        Categorify(
            replace=True, use_frequency=True, freq_threshold=int(args.freq_thresh)
        )
    )
...
new_train_set = [
    os.path.join(out_train, x) for x in os.listdir(out_train) if x.endswith("parquet")
]
new_valid_set = [
    os.path.join(out_valid, x) for x in os.listdir(out_valid) if x.endswith("parquet")
]

t_batch_sets = [
    FileItrDataset(
        x, names=cols, engine=args.in_file_type, batch_size=int(args.batch_size)
    )
...
start = time()
model = TabularModel(
    emb_szs=embeddings, n_cont=len(cont_names), out_sz=2, layers=[512, 256]
)
