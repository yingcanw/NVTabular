...
train_set = [
    os.path.join(data_path, x) for x in os.listdir(data_path) if x.endswith("parquet")
]
cont_names = ["I" + str(x) for x in range(1, 14)]
cat_names = ["C" + str(x) for x in range(1, 24)]
cols = ["label"] + cont_names + cat_names
proc = nvt.Workflow(
    cat_names=cat_names, cont_names=cont_names, label_name=["label"], to_cpu=False
)
proc.finalize()
...
for batch_size in [2 ** i for i in range(9, 26, 1)]:
    ...
    t_batch_sets = [
        FileItrDataset(
            x, names=cols, engine=args.in_file_type, batch_size=batch_size, sep="\t"
        )
        for x in train_set
    ]
    t_chain = torch.utils.data.ChainDataset(t_batch_sets)
    t_data = DLDataLoader(
        t_chain, collate_fn=dlc.gdf_col, pin_memory=False, num_workers=0
    )
