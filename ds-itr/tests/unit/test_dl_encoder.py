...
@pytest.mark.parametrize("use_filter_freq", [True, False])
def test_dl_encoder_fit_transform_fim(datasets, batch, dskey, use_filter_freq):
...
def test_dl_encoder_fit_transform_ltm(datasets_ltm, batch, dskey):
    ...
    enc = encoder.DLLabelEncoder("name-string", path=str(datasets_ltm['cats']), limit_frac=0.000001)
